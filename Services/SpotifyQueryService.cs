...
public class SpotifyQueryService(AppDbContext context)
{
    ...
    {
        IQueryable<Spotify> q = ApplyFilters(BaseQuery, filters);
        ...
        List<Spotify> items = await q.Skip((page - 1) * pageSize).Take(pageSize).ToListAsync();
    ...
    public async Task<IReadOnlyList<TopTrackDto>> GetTopTracksAsync(int limit, string countBy, SpotifyFilterParams f)
    {
        ...
        var agg = await q
            .GroupBy(x => new
                { x.master_metadata_track_name, x.master_metadata_album_artist_name, x.spotify_track_uri })
            .Select(g => new
            {
    ...
    public async Task<IReadOnlyList<HeatCellDto>> GetHeatmapAsync(SpotifyFilterParams f)
    {
        ...
        List<HeatCellDto> data = await q
            .GroupBy(x => new { x.ts.Date, x.ts.Hour })
